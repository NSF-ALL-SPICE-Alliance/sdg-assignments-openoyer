...
```{r, warning=FALSE, message=FALSE}
goal_3_and_6_plot <- ggplot(sdr_data, aes(x = goal_3_score, 
...
```
```{r}
ggplotly(goal_3_and_6_plot)
```

Based on the graph and the correlation coefficient there is a positive correlation between sdg goals 3 and 6. Countries that have a high score for sdg 3 (good health and well being) are likely to also have a high score for sdg 6 (clean water and sanitation). I think this correlation exists because when countries struggle with clean water and sanitation this can lead to many different health issues. I did notice a few outliers on the graph, one of them being Malta which has a high sdg 3 score(91.2), but a relatively low sdg 6 score(48.7).

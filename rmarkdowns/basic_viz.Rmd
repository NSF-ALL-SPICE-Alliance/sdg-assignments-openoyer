...
```{r}
ggplot(data = sdr_data, aes(x= goal_7_score, y= goal_1_score, color= goal_4_score))+
  geom_point()
```
Based on the scatterplot I created there seems to be a connection between goal 1, 4, and 7. It seems that countries who score high with sdg 4 (quality education) tend to also score high with sdg 1 (no poverty) and 7 (affordable and clean energy), while countries scoring low with sdg 4 seem to also score low with sdg 1 and 7. These connections tell me that quality education directly relates to poverty levels and energy access; meaning that receiving a quality education means you are less likely experience poverty and have more access to affordable and clean energy.
...
```{r}
ggplot(sdr_data, aes(x= goal_8_score,
fill=regions_used_for_the_sdr))+
  geom_histogram(bins = 30, color="black")
```
Based on the histogram it seems countries who have a higher sdg 8 (decent work and economic growth) score are mostly countries in the OECD region. The middle of the graph is a mix of many regions. The lower end of the sdg 8 scores are mostly from the Sub-Saharan Africa region, but the very lowest is actually from E Europe and C Asia.

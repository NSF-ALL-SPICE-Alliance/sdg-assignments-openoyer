...
```{r}
importance_df <- as.data.frame(rf_cleanf$importance)
```
...
ggplot(importance_df_top_10, aes(x = `%IncMSE`, y = reorder(variable, `%IncMSE`))) +
  ...
  theme_minimal() +
  labs(title = "Most Important Variables in Predicting Clean Fuel Access",
       subtitle = "Top 10",
       y = "SDG Indicator",
...
```{r}
pdp::partial(rf_cleanfuel, pred.var = "normalized_score_sdg12_ewaste", plot = TRUE)
```
...
```{r}
pd <- pdp::partial(rf_cleanfuel, pred.var = c("normalized_score_sdg12_ewaste", "normalized_score_sdg13_co2gcp"))
# Default PDP
plotPartial(pd)
```

...
goal_3_and_4_scatterplot <- ggplot(data = sdr_data, aes(x= goal_3_score, y = goal_4_score, color = regions_used_for_the_sdr)) +
  ...
  theme_minimal()+
  scale_fill_viridis_d()+
  labs(title = "Goal 3 and 4 Relation",
       x = "SDG 3 Score",
       y = "SDG 4 Score",
       fill = "Region")
...
leaflet(sdr_data_world_joined) %>% 
  ...
  addPolygons(stroke = FALSE, fillOpacity = 0.5, smoothFactor = 0.5, color = ~colorQuantile("RdYlBu", goal_4_score)(goal_4_score), label = mytext)
```
